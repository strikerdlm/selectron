...
The figures above are generated from a single deterministic worked example: candidate alias DEMO-01, evaluated under the Medium accessibility tier (K = 10 active criteria, as shown in [@fig:tiers]) against the eight-mission analog catalog. DEMO-01 is not a real applicant; the alias denotes a synthetic baseline candidate whose raw scores are set to the midpoint of each criterion's instrument scale. This choice is deliberate: it locates DEMO-01 at the center of the scoring simplex and serves as a calibration reference — a candidate who is neither strong nor weak on any measured dimension. All pipeline stages are seeded at 0xc0ffee (decimal 12,648,430), making every number in this section exactly reproducible from the repository at the commit recorded in the figure captions.

The pipeline overview is shown in [@fig:pipeline]. The two-stage architecture proceeds left to right: Stage A receives the K active criterion scores, runs T = 5,000 Dirichlet weight draws via the Gamma decomposition (§2.2), and returns a posterior distribution over the total composite score S_i together with 90 % and 95 % credible intervals and an effective sample size (ESS) diagnostic. Stage B then selects an analog mission from the catalog, assembles a synthetic crew by cloning the candidate profile to the mission's crew size, and runs T = 100,000 IMM-style trials to produce a posterior over the Crew Health Index χ, the early-termination probability P(χ < χ*), and the expected lost crew-days E[LCD]. The χ posterior is then handed off to the NASA HSRB Likelihood × Consequence mapping (§2.4) to yield a risk color. The medium tier exposes the two commercial-license criteria — the NASA Cognition Battery composite score and the MSCEIT emotional-intelligence standard score — in addition to the eight instruments available at the minimum tier; the two hardware-gated clinical criteria (SOT-5 equilibrium score and MMPI-2-RF EID) are deactivated. The three-tier model ([@fig:tiers]) thus controls both which constructs are scored and what instruments are applied, without altering the mathematical pipeline.
...
The Stage A posterior for DEMO-01 is shown in [@fig:posterior]. The posterior mean composite score is S_i = 0.500, with a 90 % credible interval [0.500, 0.500] and a 95 % credible interval [0.500, 0.500]. The ESS for this run is 5,121, against a draw count of T = 5,000 — an ESS-to-T ratio of approximately 1.02, which confirms that the 5,000 Dirichlet draws are essentially independent (IID Dirichlet draws have lag-1 autocorrelation ρ₁ ≈ 0 by construction, so ESS ≈ T within finite-sample fluctuation). The posterior distribution is a point mass at S_i = 0.5, not a spread distribution. This degenerate outcome is expected and informative: when all K active criterion scores are set to the scale midpoint, the normalized z-score for each criterion is z_k = 0.5 exactly, and the composite score S_i = Σ w_k · z_k = 0.5 · Σ w_k = 0.5 regardless of the particular weight draw. The Dirichlet uncertainty over weights integrates out completely when all criteria are scored identically — a mathematical invariant showing that weight uncertainty is only consequential when candidates are differentiated across criteria. For any real applicant with unequal criterion scores, the Dirichlet posterior produces a non-degenerate spread whose width reflects criterion heterogeneity and concentration strength.

[@fig:trace] shows the four-step calculation trace for a representative Dirichlet draw. Step 1 displays the raw criterion scores on their instrument-native scales (all midpoints for DEMO-01). Step 2 shows the normalized z-values in [0, 1] after the linear transformation z(x) = (x − scale.min)/(scale.max − scale.min), with the `higherIsBetter = false` flag inverting PVT-B reaction time and BDI-II scores so that higher z always denotes better performance. Step 3 shows the sampled weight vector w from Dir(α). Step 4 reports the weighted sum S_i = Σ w_k · z_k = 0.500. The trace is inspectable at the application level in the deployed browser tool; the manuscript renders it as a static panel for transparency. The closed-form Dirichlet moment check (§2.6, V&V Factor 1) compares the Monte Carlo mean and variance to the analytic values; for the flat-prior symmetric case E[S_i] = 0.5 and Var[S_i] = 0/(12·11) = 0 (identically zero since all z_k are equal), consistent with the degenerate posterior shown.
...
The χ value of 0.910 means that, for DEMO-01 on a 45-day mission with a crew of 6, the model expects approximately 9.0 % of total available person-days (270 person-days) to be lost to health incidents — corresponding to E[LCD] = 24.21 crew-days. The strong performance (χ close to 1.0) follows from two sources: DEMO-01 scores at the midpoint of all criteria, placing the vulnerability multiplier exp(β_c · z_i) near unity for all conditions that weight criterion scores; and the synthetic prior rates are conservatively set at moderate rather than high incidence, reflecting a well-supported low-risk mission profile. The P(χ < χ*) = 0 outcome is a consequence of the high χ_mean combined with the relatively low variance of the synthetic prior: under these priors, the probability mass below 0.7 is numerically zero across 100,000 trials. This is a feature of the current synthetic calibration, not a claim about the operational fleet; real-data priors from Phase-3B will produce non-zero P(χ < χ*) for high-incidence conditions.
...
This verdict has a natural operational reading: the model assigns very low likelihood of mission-level degradation (no simulated run drops below the 70 % crew-health threshold) but a non-negligible consequence if degradation were to occur (9 % of crew-days lost to incidents is categorized as significant). The L1 assignment reflects the synthetic prior's conservatism, while the C3 assignment reflects the real 45-day duration and crew-size product. The C3 consequence band is not a concern in isolation — it maps to expected operational conditions for moderate-duration analog missions — but it illustrates that a different candidate with lower composite score, or a real-data prior with higher incidence rates, would produce a wider χ credible interval and a higher P(χ < χ*), potentially escalating the likelihood level to L2 or L3 and tipping the verdict from green to yellow.

### 3.5 Cross-mission comparison

[@fig:missions] shows the eight-mission comparison for DEMO-01 at Medium tier. Each panel presents the χ-posterior histogram and the NASA HSRB LxC chip for one mission in the catalog, sorted by ascending duration from 7 days (MDRS one-week rotation) to 520 days (Mars500). The figure uses synthetic chi samples generated by the deterministic seeded formula in `TestFigureHost.tsx` (seed base 0xc0ffee incremented by catalog index) to ensure pixel-for-pixel reproducibility across browser environments without requiring IndexedDB persistence.
...
The non-monotonic pattern of consequence with duration — where the 22-day and 45-day missions escalate to C4 while the 90-day and longer missions return to C3 or C2 — reflects the synthetic prior's mission-index parameterization (base chi falls with catalog index), rather than an empirically calibrated duration effect. Under Phase-3B real-data priors, the direction of the duration effect is expected to reverse for most conditions: longer missions accumulate more total incidence events, and the QTL-to-available-person-days ratio should increase with duration for conditions with positive time-exposure rates. The comparison panel serves as a cross-mission sensitivity screen: programs with budget to evaluate only one mission profile can use the green/yellow/red chip per panel to prioritize which mission risk profiles warrant further investigation before candidate deployment.
