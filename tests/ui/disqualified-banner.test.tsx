...
function DisqualifiedBanner({ gate }: { gate: GateResult }) {
  if (gate.verdict !== "disqualified") return null;
  return (
    <div
      role="alert"
...
describe("demo-threshold review banner", () => {
  ...
  it("does NOT render banner when gate verdict is qualified", () => {
    const gate: GateResult = {
      verdict: "qualified",
      failedGates: [],
      evaluated: ["psych.mmpi2rf_eid", "cognitive.nasa_cognition_battery"],
    ...
    render(<DisqualifiedBanner gate={gate} />);
    expect(screen.queryByTestId("disqualified-banner")).toBeNull();
  });
});
