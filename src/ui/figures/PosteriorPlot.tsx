...
// CI₉₀ shaded band: very light amber fill.
const CI_AREA_COLOR = "rgba(217, 119, 6, 0.10)";
...
export function PosteriorPlot({ posterior, seed = 0xc0ffee, alias = "—", accessTier = "minimum" }: Props) {
  ...
  const { centers, counts } = buildHistogram(posterior.samples, N_BINS);
  ...
  const meanLabel   = closestCenter(posterior.mean);
  const ci90LoLabel = closestCenter(posterior.ci90[0]);
  const ci90HiLabel = closestCenter(posterior.ci90[1]);
  ...
  return (
    <>
      <ReactEChartsCore
        ...
        notMerge
      />
      <FigureCaption block={f1Caption(posterior, seed, alias, accessTier)} />
    </>
  );
}
