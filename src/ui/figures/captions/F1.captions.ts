...
export function f1Caption(posterior: Posterior, seed: number, alias: string, accessTier: AccessTier): CaptionBlock {
  return {
    figureId: "F1",
    oneLine: `Uncertain-weight MCDA score distribution for ${alias} (n = ${posterior.samples.length.toLocaleString()} draws; tier · ${TIER_LABEL[accessTier]}).`,
    methods:
      ...
      `normalisation to [0,1]). CI90 shaded; distribution mean overlaid as dashed line. ` +
      `Software: ${SOFTWARE_VERSION}.`,
    source:
      `Synthetic-iter1-engine. Computed in-browser at render time; no DB cache. ` +
      `Accessibility tier: ${TIER_LABEL[accessTier]}.`,
    reproducibility: `seed=${seed}, draws=${posterior.samples.length}, alpha=[1,…,1].`,
  };
}
