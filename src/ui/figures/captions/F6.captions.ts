...
export function f6Caption(args: { totalScore: number }): CaptionBlock {
  return {
    ...
    methods:
      "Radar plot of weighted per-criterion contribution w̄_k · u(x_k), where w̄_k is the " +
      "mean Dirichlet weight (closed-form α_k/Σα_l with α=(1,…,1) → 1/K) and " +
      "u(x_k) is the normalised criterion score in [0,1].",
