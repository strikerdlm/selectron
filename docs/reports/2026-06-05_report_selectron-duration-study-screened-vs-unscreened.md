...
1. **Crew selection is, first and foremost, psychiatric-morbidity prevention.** Its largest, most duration-robust effect is on expected behavioral/psychiatric events. The **~11× figure is a bounding sensitivity contrast** between an ideal (EID T = 35) and a clinical-extreme (EID T = 90) profile — not a calibrated screened-vs-unscreened population effect (see §6 finding 2 and §8). Read as a bound, it is the gap between a 500-day campaign with a near-certain psychiatric event for the worst-case profile (99.9 %) and one where it remains less likely than not for the best-case profile (44.8 %); the real-world *screened* floor is anchored at the ~5% per-expedition disorder prevalence observed in Antarctic winter-over crews (Palinkas & Suedfeld, 2008; Lugg, 2005).
