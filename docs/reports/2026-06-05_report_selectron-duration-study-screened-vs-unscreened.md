...
**These crews are deliberate bounding extremes, not representative populations.** EID T = 90 is the clinical ceiling of the scale (+4 SD above the normative mean of 50; a profile consistent with active internalizing pathology that no selection process would pass); EID T = 35 is a near-ideal screened profile (−1.5 SD, well below the T ≥ 65 clinical-elevation threshold). The screened/unscreened contrast therefore **bounds the sensitivity** of the selection criterion between a best- and worst-case profile; it is *not* a sample of any real unscreened cohort — no analog mission has ever flown an unscreened control arm. Interpret every screened-vs-unscreened ratio below (especially the ~11× psychiatric figure) accordingly (see §6, finding 2).
...
2. **The ~11× psych ratio is internally coherent — and is a bounding contrast, not a population estimate.** The ratio is quantitatively explained by the vulnerability math: for a psychiatric condition coupled to both emotional stability and EID (β = −0.4), the screened crew's multipliers are exp(−0.4·1.6) × exp(−0.4·1.78) ≈ 0.26 and the unscreened crew's exp(0.8) × exp(0.27) ≈ 2.91 — a predicted ratio of **≈ 11.2×**, matching the observed 10.8–11.9×. This is an *internal-coherence* check (the math is visible in the output), **not external validation.** The two crews are deliberate extremes (EID T = 35, −1.5 SD, vs EID T = 90, +4 SD — the clinical ceiling), and no analog mission has ever flown an unscreened control arm — ethically none ever will — so the unscreened limb is a counterfactual. The 11× therefore **bounds the sensitivity** of the selection criterion between a best- and worst-case profile; it does not estimate any real unscreened cohort. The empirical anchor is the **screened** arm: roughly **5% per-expedition DSM/ICD psychiatric-disorder prevalence is repeatedly reported for *already-screened* Antarctic winter-over crews** (Palinkas & Suedfeld, 2008; Lugg, 2005; Friedman & Bui, 2017) — a residual floor that selection cannot drive to zero. The absolute multiplier is also contingent on the expert-elicited psychiatric β (a ±50% perturbation of β moves it materially while preserving the ordering; see §8); the robust, literature-supported claim is the *qualitative* one — psychiatric/behavioral burden is the single most selection-sensitive endpoint — not the specific number.
...
4. **Where the fixture contrast appears:** p(≥ 1 psychological event) for the unscreened fixture crosses **25 % by 22 d**, **50 % by ~50 d**, **80 % by 120 d**, and is **virtually certain (99.9 %) at 500 d** — while the screened fixture stays below 15 % out to 120 d. Medical events, by contrast, are near-certain for either fixture on any campaign ≥ 45 d (p ≥ 1 medical event ≈ 100 %): the fixture contrast does not remove medical events, but it produces a ~19 % lower medical event count and an order-of-magnitude lower psychiatric burden in this scenario run. Dominant psych drivers in the unscreened fixture at all durations: **late-insomnia > anxiety > depression**.

## 7. Conclusions

1. **The crew-quality fixture contrast primarily moves psychiatric-morbidity outputs.** Its largest, most duration-robust effect is on expected behavioral/psychiatric events. The **~11× figure is a bounding sensitivity contrast** between an ideal (EID T = 35) and a clinical-extreme (EID T = 90) profile — not a calibrated screened-vs-unscreened population effect (see §6 finding 2 and §8). Read as a bound, it is the gap between a 500-day campaign with a near-certain psychiatric event for the worst-case profile (99.9 %) and one where it remains less likely than not for the best-case profile (44.8 %); the real-world *screened* floor is contextualized by the ~5% per-expedition disorder prevalence observed in Antarctic winter-over crews (Palinkas & Suedfeld, 2008; Lugg, 2005), but this is not Selectron calibration.
...
3. **Mission duration dominates everything** in this historical scenario run: the crew-quality fixture contrast does not compensate for exposure time. pEVAC grows superlinearly with duration (kit depletion), and either fixture on a ≥ 45-day campaign will almost surely generate medical events.
4. These results reinforce the §4 assessment from the archetype report: TME/CHI/psych-event burden are the metrics affected by the fixture contrast; the evac/LOCL tails are kit- and severity-dominated.
...
- FAMILY_BETA values are expert-elicited, sensitivity-audited, not fitted to analog outcome data (none exists); the ~11× psych ratio inherits that elicitation and is reported as a best-/worst-case **bounding contrast** (EID T = 35 vs T = 90), not an estimate of any real unscreened cohort — no analog mission has run an unscreened control arm. The *screened* arm is anchored empirically at the ~5% per-expedition psychiatric-disorder prevalence observed in already-screened Antarctic winter-over crews (Palinkas & Suedfeld, 2008; Lugg, 2005; Friedman & Bui, 2017).
...
The ~5% per-expedition prevalence in *screened* polar winter-over crews is the empirical floor for the screened arm; the unscreened arm (EID T = 90) is a synthetic clinical-extreme counterfactual, not an observed cohort. See `docs/reports/2026-06-05_literature-review_selectron-imm-defensibility.md` §1 for the full defensibility assessment of the 11× figure.
