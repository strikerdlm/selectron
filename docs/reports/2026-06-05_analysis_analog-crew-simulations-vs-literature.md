...
Psychiatric event ratio (unscreened/screened) is **10.8–11.9× across all 7 durations**. The report's quantitative check is correct: for a psychiatric condition coupled to both emotional stability and EID, the `exp(β·z)` math gives screened ≈ 0.26, unscreened ≈ 2.91, ratio ≈ 11.2×. **The model is not just discriminating; it is discriminating by exactly the mechanism it claims to use.** This is the strongest possible internal-validation result — the math is visible in the data.
...
**Verdict:** the engine's **qualitative behavior is sound and matches the published analog-mission science on the dimensions the catalog can represent.** The three coverage gaps (Antarctic catalog, conscientiousness, interpersonal conflict) are **documented in the reports and acknowledged as future work** — they are not silent failures.

The strongest feature of the model is the **kit-depletion → superlinear pEVAC** behavior, which emerges naturally from `treatment.ts` and reproduces a planning-relevant fact of real analog operations that the literature emphasizes. The weakest feature is the 11× psych ratio's dependence on a non-fittable β, which is honestly disclosed and bounded by sensitivity.

---

## 7. Specific recommendations for the manuscript (ASR submission)

1. **Lead with the 11× psych ratio and the superlinear pEVAC.** These are the two findings most aligned with the analog-mission literature and most useful for planning. The current report buries them in §4 of the duration study; the manuscript abstract should lead with the categorical (event/no-event) impact of screening at long durations.
...
**The MCMC simulations behave the way the scientific literature says long-duration isolation and confinement should behave, on every dimension the catalog can represent, and the gaps are documented and bounded.** The reports are not over-claiming. The model is a sound planning tool for analog-mission crew selection, with the calibration candidates (EID neutral point, conscientiousness coverage, Antarctic catalog) properly deferred to post-submission work. The strongest individual finding — the **11× psychiatric-event reduction from screening, stable across 7–500 d** — is internally coherent (math is visible in the data) and externally consistent with the qualitative literature (selection is the highest-yield lever). The **kit-depletion superlinear pEVAC** is a non-obvious emergent behavior that the literature supports.
...
Three concrete actions for ASR submission:
- Lead the abstract with the 11× finding + kit-depletion superlinearity
