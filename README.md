...
**Validation status within Earth-analog + LEO-ISS scope (current, 2026-05-22 priors-rev3-b + analog-scope-down):** K15 Table 1 reproduction is partial-but-substantially-improved. **5 of 12 K15 metrics now within CI₉₅** (was 1 of 12 pre-rev3-b): all 3 TME ✓ (≤Δ 8.2 across all scenarios), 'none' CHI ✓ (Δ +5.68), 'unlimited' CHI ✓ (Δ −0.10, essentially perfect). The fix was a single-knob tier-B incidence multiplier (`global_calibration.tierB_multiplier = 0.55`) — diagnostic showed tier-B dominated TME at ~65 % so multi-knob coordinate descent was unnecessary. Bonus: discovered + fixed a stochastic-rounding bug in the original tier-C multiplier path (Math.round biased small counts → near-no-op for count=1; replaced with floor+Bernoulli(frac)). In-scope residuals: pEVAC + pLOCL under-elicitation on the 'none' (no kit) scenario (rev3-c closed-form rescale of `untreated.p_evac` / `untreated.p_locl`), and issHMS CHI Δ −19.29 (third axis, not fixable by incidence scaling — needs severity tuning or further kit-coverage work). **Out-of-scope:** Mars (TM21) and Artemis missions are catalogued in [`docs/future_features.md`](docs/future_features.md) with their structural prerequisites and are NOT part of the v1 validation gate. Full delta table + phasing in [`docs/iter5_priors_rev3_strategy.md`](docs/iter5_priors_rev3_strategy.md) §7 and [`docs/iter3_vv_dossier.md`](docs/iter3_vv_dossier.md) §5.
...
1. **IMM priors-rev3-c — closed-form per-event `untreated.p_evac` / `untreated.p_locl` rescale** (HIGH PRIORITY now that Mars is out of scope; gates IMM-86 K15 reproduction test on the 'none' scenario). Target: lift `none` pEVAC from 13.7 % to K15's 66.9 % and pLOCL from 0.45 % to 2.89 %. Closed-form: given calibrated event count N (correct per rev3-b), target per-event `p_target ≈ 1 − (1 − P_K15)^(1/N)`. Apply uniformly to `untreated.p_evac.mode` and `untreated.p_locl.mode` preserving Beta-Pert shape. May lift issHMS pEVAC slightly above ref (over-correction signal already present) — acceptable trade for closing the larger 'none' gap.
2. **Per-condition source audit of top-20 tier-B contributors** — spot-check each of the highest-contribution tier-B priors against the primary literature source. Replaces the blanket tier-B 0.55 multiplier with per-condition corrections where the source actually warrants it. Higher-rigor alternative to the rev3-b blanket scalar. See [`docs/iter5_scientific_limitations.md`](docs/iter5_scientific_limitations.md) §3.2.
