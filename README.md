...
**Validation status within Earth-analog + LEO-ISS scope (current, 2026-05-22 priors-rev3-c + IMM→LxC adapter):** K15 Table 1 reproduction is further improved. **6 of 12 K15 metrics now within CI₉₅** (was 5/12 post-rev3-b; was 1/12 pre-rev3-b): all 3 TME ✓ ('none' Δ +0.84 dead-on; issHMS Δ −6.30; unlimited Δ −5.64), 'none' CHI ✓, **'unlimited' CHI essentially perfect** (Δ +0.25). rev3-c replaced the blanket tier-B multiplier with **per-condition source-cited priors** for 5 high-impact conditions: `dental-caries` (G12 Bayesian submarine + LSAH chain), `late-insomnia` (Mars-500 Basner 2014 + SIRIUS-21 + WOTR15), `depression` (Palinkas 2004 + Hong 2022 Antarctic), `respiratory-infection` (Bhatia 2012 Maitri + Pattarini 2016 McMurdo), `skin-rash` (Pattarini 2016 + WOTR15). 27 distinct primary citations consolidated by 3 parallel research agents — full table in [`research/_priors_rev3c_synthesis.md`](research/_priors_rev3c_synthesis.md). The **IMM Calculator output now feeds the NASA HSRB LxC matrix verdict** via `src/imm/lxc.ts::assessIMMLxC` — Crew Composition view shows the colour-coded L×C cell + JSC-66705 verbatim labels + crew-gate fast-fail. In-scope residuals: issHMS CHI Δ −16.11 (severity-axis, not fixable by incidence scaling; needs per-condition `fi_cp` × `dt_cp_hours` audit), 'none' pEVAC under-elicited (K15-model-construct artifact — see scientific limitations §4.1 for the decision rationale). **Out-of-scope:** Mars (TM21) and Artemis missions are catalogued in [`docs/future_features.md`](docs/future_features.md). Full delta tables in [`docs/iter5_priors_rev3_strategy.md`](docs/iter5_priors_rev3_strategy.md) §7-§8 and [`docs/iter3_vv_dossier.md`](docs/iter3_vv_dossier.md) §5.
