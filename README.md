...
0. **Python offline calibration pipeline DONE** (`python/`). PyMC NUTS Gamma-Poisson fitter + K15 validator + atomic priors writer + Sobol/Morris sensitivity. **27 of 30 Gamma-Poisson tier-B conditions now fittable** from MCP-searched literature: spaceflight (Tran 2023, Crucian 2016, Scheuring 2009, Gilkey 2012 LSAH), submarine (Deutsch 2008, Toklu 2004), Antarctic (Bhatia 2012, Pattarini 2016, Palinkas 2004), military (Orsello 2013, Hauret 2012, CARDIA), and terrestrial epidemiology. CLI: `cd python && source .venv/bin/activate && python -m selectron --dry-run`. 50 fast tests pass.
...
| **3 of 30 Gamma-Poisson tier-B priors lack individual evidence** (elbow/hip/wrist-sprain-strain). All others now have at least 1 literature-backed observation in the Python pipeline. | Medium | 27/30 conditions fittable via `python -m selectron`; 11 Beta-Bernoulli conditions deferred to v1.1. |
