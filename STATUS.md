...
**Next action:** (a) Diego reviews `research/02_criterion_taxonomy.md` and ratifies into `docs/criteria.md` (Iter-2 start gate); (b) Diego performs Task 17 manual UI sanity at http://localhost:5173 — controller then issues the `release(iter1)` commit; (c) **Iter-3 Phase 3.0 evidence corpus is in.** Tier-1 firecrawl (`dee32b1`): G12, M18, A22, K15, W14 (1-p poster), S20 (abstract). Tier-2 zotero-pdf-ocr (`8910881`): 10 IMM-related papers OCRed + S20 full-text backfill (now 54 KB with all numeric anchors). Corpus state: **16 IMM/V&V papers** across `research/imm_sources/{methods,architecture,validation,zotero_imm}/`. Memory MCP holds ~50 verbatim math observations across 17 entities. **Diego decides next:** (i) Phase 3.0 Task 30 scite citation graph, (ii) Phase 3.0 Task 31 paper-search follow-on, OR (iii) skip directly to ratifying Iter-2 criteria taxonomy.
