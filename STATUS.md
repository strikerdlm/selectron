...
**Last updated:** 2026-05-19 05:48 UTC
...
**Next action:** Task 56 — `src/ui/components/ConditionContribution.tsx` (stacked bar of per-condition QTL) + `src/ui/components/RiskHistogram.tsx` (ECharts histogram of CHI posterior, reusing the core registration from Iter-1 `PosteriorPlot.tsx`). Iter-3 Phase 3D is underway: Tasks 54 (MissionPicker) + 55 (RiskCard) DONE; remaining 3D backlog is Tasks 56 + 57 (App.tsx wiring). **Phase 3A/3B priors** (incidence_rates.csv curation → PyMC fit → real priors.json) is still outstanding and may be sequenced before Task 57 so the wired UI consumes real posteriors instead of synthetic in-test priors. **Iter-1/2 backlog still open:** Diego ratifies `research/02_criterion_taxonomy.md` → `docs/criteria.md` (Iter-2 start); Diego signs off Task 17 manual UI sanity at http://localhost:5173 (Iter-1 release).
...
| 55 | `src/ui/components/RiskCard.tsx` | **DONE** | `8c6bc4d` | Plan supplied no verbatim heredoc — only "mirrors ScoreCard.tsx layout". Implementer transposed ScoreCard's chrome (panel, hairline, dl grid, sharpness gauge) onto the three RiskPosterior quantities (CHI mean+CI90+CI95 as headline; pEarlyTermination mean+CI90; expectedLostCrewDays mean+CI90 rendered in days). ESS + trials added to dl grid for convergence transparency. Sharpness gauge reused over CHI CI90 width with the same 0.3 cap as ScoreCard (decorative, intentional layout mirroring). "1 = nominal" sub-caption disambiguates CHI's higher-is-better semantics. Typecheck clean; smoke deferred to Task 57 / Task 58. |
