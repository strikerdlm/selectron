...
**Next action:** **Iter-3 Phase 3C engine math is feature-complete (Tasks 44–53 DONE).** The path branches to (a) Phase 3D UI (Tasks 54–57 — MissionPicker, RiskCard, ConditionContribution, RiskHistogram, App.tsx wiring) or (b) Phase 3A/3B prerequisites (incidence_rates.csv curation → PyMC fit → real priors.json). **Iter-3 engine math** is currently working off synthetic in-test priors; the simulator runs end-to-end (4-step IMM trial loop wired, 106 tests green) but priors.json is still a placeholder concern. **Iter-1/2 backlog still open:** Diego ratifies `research/02_criterion_taxonomy.md` → `docs/criteria.md` (Iter-2 start); Diego signs off Task 17 manual UI sanity at http://localhost:5173 (Iter-1 release).
